...
<script setup>
    ...
    const followers = ref([])  // 가져온 팔로워 리스트
    ...
    try {
        const res = await fetch(`http://localhost:3001/follow?follower_target_member_id=${userId}`)
        const followData = await res.json()

        // followData 안에는 following_member 안에 유저 정보가 있어!
        followers.value = followData.map(item => ({
        id: item.following_member.id,
        nickname: item.following_member.nickname,
        avatar: item.following_member.profile_image || profileImage,
        bio: item.following_member.bio || '',
        }))
    } catch (error) {
